...
public class PawnBehavior {
	...
	public enum PawnSMPlaying {
		Null, Setup, InMiddle, AtNorthEdge, AtEastEdge, AtSouthEdge, AtWestEdge
	}
	...
	public String getPawnSMFullName() {
		...
		if (pawnSMPlaying != PawnSMPlaying.Null) {
			answer += "." + pawnSMPlaying.toString();
		}
		return answer;
	}
	...
	public boolean startGame() {
		...
		case Setup:
			...
			if (getCurrentPawnColumn() == 9 && getCurrentGame().getGameStatus().equals(GameStatus.Running)
					&& getCurrentPawnRow() != 1 && getCurrentPawnRow() != 9) {
				exitPawnSMPlaying();
			...
			if (getCurrentPawnColumn() == 1 && getCurrentGame().getGameStatus().equals(GameStatus.Running)
					&& getCurrentPawnRow() != 1 && getCurrentPawnRow() != 9) {
				exitPawnSMPlaying();
			...
			if (getCurrentPawnColumn() <= 8 && getCurrentPawnColumn() >= 2 && getCurrentPawnRow() <= 8
					&& getCurrentPawnRow() >= 2 && getCurrentGame().getGameStatus().equals(GameStatus.Running)) {
				exitPawnSMPlaying();
	...
	private void setPawnSM(PawnSM aPawnSM) {
		...
		case Playing:
			if (pawnSMPlaying == PawnSMPlaying.Null) {
				setPawnSMPlaying(PawnSMPlaying.Setup);
			}
			break;
		case GameOver:
	...
	private void setPawnSMPlaying(PawnSMPlaying aPawnSMPlaying) {
		...
		if (pawnSM != PawnSM.Playing && aPawnSMPlaying != PawnSMPlaying.Null) {
			setPawnSM(PawnSM.Playing);
		}
	 ...
	 * 
	 * 
	 * Changes the game status of the game when a player is in a winning position
	 * (Will be modified in future deliverables)
	 * 
	...
	public void checkWinningMove() {
		if (player.hasGameAsWhite() && currentGame.getCurrentPosition().getWhitePosition().getTile().getRow() == 1) {
			currentGame.setGameStatus(GameStatus.WhiteWon);
				...
				&& currentGame.getCurrentPosition().getBlackPosition().getTile().getRow() == 9) {
	 ...
	 * @author Remi Carriere
	 * 
	 * @return True if white won, false otherwise
	 * 
	 */
	 ...
	 * @author Remi Carriere
	 * 
	 * @return True if black won, false otherwise
	 * 
	 */
	 ...
	 *            Direction to move
	 * 
	 * @return an array containing [targetRow, targetCol]
	 * 
	 */
	...
	public void illegalMove() {
		// We cant throw an exception here since doing so would set the current state to
		// null and crash the state machine. Alse, the UI does not allow a user to
